...
namespace TP1_Generador_de_numeros_pseudoaleatoreos.Controllers
{
    class ControllerBondadAjuste
    {
        ...
        private void acumular()
        {
            double acum;
            int puntero = 1;
            //int vueltas = frecuencias_esperadas.Length;
            List<double> frecuencias_esperadas_acum = new List<double>();
            for (int i = 0; i < frecuencias_esperadas.Length; i++)
            {
                if(frecuencias_esperadas[i] < 5)
                {
                    acum = frecuencias_esperadas[i];
                    while ((acum+frecuencias_esperadas[puntero]) < 5)
                    {
                        acum += frecuencias_esperadas[puntero];
                        puntero++;
                        i++;
                    }
                    frecuencias_esperadas_acum.Add(acum);
                }
                else
                {
                    frecuencias_esperadas_acum.Add(frecuencias_esperadas[i]);
                }
            }
